#!/usr/bin/env node
// v1.1 re-run driver — identical to the original base-rate driver-list.mjs
// except outFile lands next to the LIST file (this session's scratchpad),
// not next to the repo clone. Usage: node driver-list.mjs <repoDir> <listFile> <outTag>
...
const outFile = path.join(path.dirname(path.resolve(listFile)), `results-${tag}.jsonl`);
