...
5. **External PROVENANCE segregation** *(rechecked 2026-07-01 — initial "firewall not enforced" flag was overstated)* — VERIFIED there is no `trust:external` entry in any of 37 notebooks, so no foraged floor content is loadable; **PARA-901's structural context-firewall is accurate** (`notebook-loader.ts:117,131-144` has no read-filter *by design*; the guard is journal-vs-notebook + the `/class` gate). Do **not** add a read-filter. Narrower valid point: certified entries retain a `source:external` *provenance* concept, so segregate external-origin cert **outcomes** from any published `survivalShape`.
