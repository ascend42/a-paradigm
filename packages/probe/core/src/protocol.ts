 ...
 * Generates the cursorrules section for paradigm scan
 */
...
export function getScanProtocol(options: ScanProtocolOptions = {}): string {
  const {
    indexPath = '.paradigm/scan-index.json or .paradigm-scan-index.json',
    enableAllModes = true,
    customInstructions = [],
  } = options;
  ...
  lines.push('## Paradigm Scan Protocol');
  lines.push('');
  lines.push('When the user says "**paradigm scan**" with an image attached:');
  lines.push('');
  ...
  lines.push('');
  lines.push(`### 2. Cross-Reference with Paradigm Index`);
  lines.push('');
  ...
  lines.push('');
  lines.push('```markdown');
  lines.push('## 🔭 Paradigm Scan Results');
  lines.push('');
  ...
  if (enableAllModes) {
    ...
    lines.push('|------|---------|----------|');
    lines.push('| `paradigm scan` | Default | Map any image to paradigm elements |');
    lines.push('| `paradigm scan ui` | Screenshot | Map running app screenshot to code |');
    lines.push('| `paradigm scan design` | Mockup | Gap analysis vs. design files |');
    lines.push('| `paradigm scan error` | Error screenshot | Find related error handlers |');
    lines.push('| `paradigm scan arch` | Architecture diagram | Map diagram boxes to features |');
    lines.push('| `paradigm scan flow` | Flow diagram | Match steps to `$flow` definitions |');
    lines.push('| `paradigm scan diff` | Two images | Show what paradigm elements changed |');
    lines.push('');
  }
...
export function getMinimalScanProtocol(): string {
  return `## Paradigm Scan

When user says "paradigm scan" with an image:
1. Identify UI elements in the image
2. Map to paradigm symbols using \`.paradigm/scan-index.json\`
