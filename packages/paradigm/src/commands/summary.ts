/**
 * paradigm summary - Generate project.md with stats
 */
...
export async function summaryCommand() {
  ...
  // Config
  if (fs.existsSync(path.join(paradigmDir, 'config.yaml'))) {
  ...
  // Docs
  const docsExist = fs.existsSync(path.join(paradigmDir, 'docs'));
  ...
  // Scan index
  const scanIndexExists = fs.existsSync(path.join(paradigmDir, 'scan-index.json'))
    || fs.existsSync(path.join(cwd, '.paradigm-scan-index.json'));
  ...
  lines.push(`**Paradigm Version:** 0.2.0`);
  lines.push('');
  ...
  lines.push('---');
  lines.push('*Generated by `paradigm summary`*');

  // Write project.md
  const outputPath = path.join(paradigmDir, 'project.md');
