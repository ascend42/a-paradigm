 ...
 */
const TUTORIAL_STATE_FILE = '.paradigm/tutorial/state.json';

/**
 * Tutorial curriculum file
 */
const CURRICULUM_FILE = '.paradigm/tutorial/curriculum.yaml';
...
async function validateCheckpoint(rootDir: string, stepId: string, curriculum: Curriculum): Promise<boolean> {
  ...
  for (const checkpoint of step.checkpoints || []) {
    switch (checkpoint.type) {
      ...
      case 'symbol-count': {
        // This would require running paradigm commands
        // Simplified for now
        break;
      }
      case 'gate-count': {
        // This would require parsing portal.yaml
        // Simplified for now
        break;
      }
