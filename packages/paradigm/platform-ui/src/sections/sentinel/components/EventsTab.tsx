/**
 * EventsTab — Schema-adaptive generic event viewer
 *
 * Ported from sentinel/ui/src/views/EventsView.tsx with:
 * - useSentinelEventsStore / useSentinelSchemasStore
 * - No direct WS connection
 */

import { useEffect, useRef, useState, useCallback, type CSSProperties } from 'react';
import { useSentinelEventsStore, type GenericEvent } from '../store/sentinelEventsStore';
import { useSentinelSchemasStore, type StoredSchema } from '../store/sentinelSchemasStore';
...
function getCategoryColor(category: string, schema?: StoredSchema): string {
  ...
  return DEFAULT_CATEGORY_COLORS[category] || 'var(--p-text-muted)';
}

// ---- Schema Selector ----

function SchemaSelector() {
  const { schemas, selectedSchemaId, selectSchema, fetchSchemas, loading } = useSentinelSchemasStore();
  ...
  if (schemas.length === 0) {
    return (
      <div className="events-empty-state">
        ...
        <p className="hint">
          Applications register schemas via <code>POST /api/sentinel/schemas</code>.
        </p>
      </div>
    );
  }
...
function EventTable({ schema }: { schema: StoredSchema }) {
  const { events, excludedTypes, excludedServices, categoryFilter, loading, toggleExcludedType, toggleExcludedService, clearAllExclusions } = useSentinelEventsStore();
...
export function EventsTab() {
  const { selectedSchemaId } = useSentinelSchemasStore();
  const schema = useSentinelSchemasStore((s) => s.getSelectedSchema());
  const { fetchEvents } = useSentinelEventsStore();
