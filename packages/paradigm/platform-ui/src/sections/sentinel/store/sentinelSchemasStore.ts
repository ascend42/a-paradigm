/**
 * Sentinel Schemas Store — Zustand store for event schema registry
 *
 * Ported from sentinel/ui/src/store/schemasStore.ts with /api/sentinel/ prefix.
 */
...
interface SentinelSchemasState {
...
export const useSentinelSchemasStore = create<SentinelSchemasState>((set, get) => ({
  schemas: [],
  ...
  fetchSchemas: async () => {
    ...
    try {
      const res = await fetch('/api/sentinel/schemas');
