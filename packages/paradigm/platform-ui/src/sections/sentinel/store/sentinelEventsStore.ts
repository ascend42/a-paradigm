/**
 * Sentinel Events Store — Zustand store for generic events + scopes
 *
 * Ported from sentinel/ui/src/store/eventsStore.ts with /api/sentinel/ prefix.
 */
...
export const useSentinelEventsStore = create<SentinelEventsState>((set, get) => ({
  events: [],
  scopes: [],
  selectedScope: null,
  ...
  fetchEvents: async (schemaId, query = {}) => {
    ...
    try {
      ...
      const res = await fetch(`/api/sentinel/events?${params}`);
  ...
  fetchScopes: async (schemaId) => {
    try {
      const res = await fetch(`/api/sentinel/events/scopes?schemaId=${encodeURIComponent(schemaId)}`);
  ...
  fetchScopeEvents: async (schemaId, scopeValue) => {
    ...
    try {
      const res = await fetch(
        `/api/sentinel/events/scope/${encodeURIComponent(scopeValue)}?schemaId=${encodeURIComponent(schemaId)}`
      );
  ...
  handleWsMessage: (msg: any) => {
    if (msg.type === 'sentinel:event' && msg.event) {
      set((state) => ({
