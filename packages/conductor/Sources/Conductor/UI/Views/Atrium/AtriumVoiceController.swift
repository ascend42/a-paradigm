...
//   off  --click-->  armed  --hear "respond"-->  composing
...
final class AtriumVoiceController: ObservableObject {
    ...
    /// Click handler for the voice icon: off → armed, or anything → off.
    func toggle() {
        switch state {
        case .off:
            arm()
    ...
    private func arm() {
        ...
        listenTask = Task { [weak self] in
            ...
            do {
                try await self.provider.start()
            } catch {
                self.log.error("Voice provider failed to start: \(error.localizedDescription)")
                self.state = .off
                return
            }
            // Continuous chunked transcription — the existing reusable loop.
            self.provider.startContinuous()
    ...
    private func handle(transcript raw: String) {
        ...
        switch state {
        case .off, .sending:
            // Ignore transcripts while off or mid-send.
            return
        ...
        case .composing:
            ...
            if let before = Self.textBeforeKeyword(text, keyword: Self.sendKeyword) {
                ...
                state = .sending
                log.info("Send phrase heard — submitting")
                onSubmit?()
