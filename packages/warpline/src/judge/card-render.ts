...
import { frameProse, envelopeProse } from '../envelope.js';
...
function renderFileBody(file: RatingCardFile): string {
  if (file.body === null) {
    return `  (absent on this side: ${file.filePath})`;
  }
  const lines = file.body.split('\n');
  if (lines.length > MAX_BODY_LINES) {
    const shown = lines.slice(0, MAX_BODY_LINES).join('\n');
    const framed = frameProse(envelopeProse(shown), { label: `source ${file.filePath}` });
  ...
  return frameProse(envelopeProse(file.body), { label: `source ${file.filePath}` });
}
...
function renderSide(side: RatingCardSide): string {
  ...
  for (const f of side.files) {
    parts.push(`  file: ${f.filePath}`, renderFileBody(f));
  }
  return parts.join('\n');
}
...
export function renderRatingCard(card: RatingCard): string {
  ...
  const parts: string[] = [
    ...
    `changed files (${card.filePaths.length}): ${card.filePaths.join(', ') || '(none)'}`,
  ];
  ...
  if (card.failingCheck !== undefined) {
    parts.push('', `── check that failed on the merged tree (name only) ──`, `  ${card.failingCheck}`);
  }
