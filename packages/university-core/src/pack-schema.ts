 ...
 *
 * COPIED verbatim into @a-company/university-core (extract-university-core spec
 * §1.5 Option B). The original lives at paradigm-mcp/src/utils/pack-schema.ts
 * and stays there for its non-university (portal/strict-mode) consumers; this
 * copy makes core's section validation byte-identical without dragging
 * portal-core into the lean core. It imports nothing but `zod` (a leaf npm dep,
 * not an @a-company drag — permitted by the lean mandate).
 *
 * Canonical home for the Section schema. Consumed by `pack-discovery.ts` for
 * manifest validation and synthesis.
 *
 * Security contract:
 *   - Schema error messages are CLASSIFIER-only — never echo field values.
 *     `pack-discovery.ts` further wraps Zod failures in `PackLoadError(
