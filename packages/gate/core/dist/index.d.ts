...
/**
 * Main Gate configuration file structure (gate.yaml)
 */
...
type WatcherEventType = 'gate:check' | 'gate:pass' | 'gate:fail' | 'prize:fire' | 'flow:start' | 'flow:progress' | 'flow:complete';
...
/**
 * Parse a gate.yaml file and return normalized config
 */
declare function parseGateConfig(configPath: string): Promise<ParsedGateConfig>;
/**
 * Parse a single .gate.yaml file containing one or more gates
 */
declare function parseGateFile(filePath: string): Promise<Gate[]>;
/**
 * Serialize a ParsedGateConfig back to YAML
 */
declare function serializeGateConfig(config: ParsedGateConfig): string;
/**
 * Get the default gate.yaml content for initialization
 */
declare function getDefaultGateConfig(): string;
/**
 * Find all gate.yaml files in a directory
 */
declare function findGateFiles(rootDir: string): Promise<string[]>;
 ...
 */
declare function validateGateConfig(config: ParsedGateConfig): ValidationResult;
/**
 * Format validation issues for console output
 */
declare function formatValidationResult(result: ValidationResult): string;

export { type DevSettings, type Flow, type Gate, type GateCheckResult, type GateConfig, type GraphData, type GraphEdge, type GraphNode, type Key, type KeyResult, type Lock, type LockResult, type ParsedGateConfig, type Prize, type ValidationIssue, type ValidationResult, type WatcherEvent, type WatcherEventType, findGateFiles, formatValidationResult, getDefaultGateConfig, parseGateConfig, parseGateFile, serializeGateConfig, validateGateConfig };
