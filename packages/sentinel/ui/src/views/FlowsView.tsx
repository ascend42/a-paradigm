/**
 * Flows View - Live flow visualization + flow composer
 *
 * Shows flow definitions from .purpose files with real-time activity.
 * Flow nodes "light up" as signals/gates fire via WebSocket events.
 * Includes a flow composer for creating new flows from existing symbols.
 */

import { useEffect, useState, useCallback } from 'react';
import { useLogsStore, type FlowEvent } from '../store/logsStore';
          ...
          return (
            <div key={i} className="flow-step-mini">
              ...
              <span
                ...
                style={{ backgroundColor: active ? STEP_COLORS[step.type] : 'var(--text-muted)' }}
...
export function FlowsView() {
  const { flowEvents, connectWebSocket, disconnectWebSocket, isLive } = useLogsStore();
  ...
  useEffect(() => {
    async function loadFlows() {
      try {
        ...
        // Extract flow symbols and build flow definitions from their data
        ...
        const defs: FlowDefinition[] = flowSymbols.map((s: any) => {
          ...
          // Also derive steps from references if no explicit steps
  ...
  if (isLoading) {
    return (
      <div className="flows-view loading-state">
