/**
 * Sentinel CLI - Formatting Utilities
 *
 * Ported from paradigm/src/commands/triage/utils/format.ts
 * with imports adjusted for standalone sentinel package.
 */
...
export function formatHeader(): string {
  return `
${chalk.cyan('╔══════════════════════════════════════════════════════════════════╗')}
${chalk.cyan('║')}                       ${chalk.bold.white('SENTINEL TRIAGE')}                              ${chalk.cyan('║')}
${chalk.cyan('╚══════════════════════════════════════════════════════════════════╝')}
`;
}
...
export function formatIncident(
  ...
  lines.push(
    chalk.gray('│   ') +
      chalk.dim(`sentinel triage resolve ${incident.id}`) +
      ' '.repeat(36 - incident.id.length) +
      chalk.gray('│')
  );
  lines.push(
    chalk.gray('│   ') +
      chalk.dim(`sentinel triage show ${incident.id} --timeline`) +
      ' '.repeat(29 - incident.id.length) +
      chalk.gray('│')
  );
