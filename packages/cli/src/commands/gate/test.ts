/**
 * horizon gate test command
 */

import * as path from 'path';
import * as fs from 'fs';
import chalk from 'chalk';
import { generateTests } from '@horizon/gate-manager';
import { scanComponents, generateComponentReport } from '@horizon/gate-manager';
import { checkGateway, validateGateway } from '@horizon/gate-manager';
import { parseGateConfig, createGate } from '@horizon/gate-core';
import { setGateClient } from '@horizon/gate-sdk';
import type { GatewayTestCase } from '@horizon/gate-manager';
...
export async function gateTestCommand(
  ...
  const gateConfigPath = path.join(rootDir, 'gate.yaml');

  if (!fs.existsSync(gateConfigPath)) {
    console.error(chalk.red(`❌ gate.yaml not found at ${gateConfigPath}`));
    console.error(chalk.gray('   Run `horizon init` or create gate.yaml manually'));
    process.exit(1);
  }
  ...
  console.log(chalk.blue('\n🧪 Gate Testing\n'));
  console.log('Usage:');
  console.log('  horizon gate test --generate          Generate test files');
  console.log('  horizon gate test --gate ^auth-required  Test specific gate');
  console.log('  horizon gate test --component           Validate component access');
  console.log('');
}
