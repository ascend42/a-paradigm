...
import { parseHorizonConfig, type HorizonConfig } from '../horizon-config.js';
...
/**
 * Load Horizon files from .horizon/ directory
 */
export function loadHorizonFiles(rootDir: string): HorizonFiles | null {
  const horizonDir = path.join(rootDir, '.horizon');
  const horizonFile = path.join(rootDir, '.horizon');
  
  // Check if .horizon is a directory (new format)
  ...
  if (fs.existsSync(horizonDir) && fs.statSync(horizonDir).isDirectory()) {
    ...
    // Legacy format: .horizon is a file
  ...
  try {
    ...
    config = parseHorizonConfig(configContent);
  } catch {
    return null;
  }
...
/**
 * Sync Horizon to IDE instruction file
 */
