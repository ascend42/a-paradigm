 ...
 *
 * Produces narration strings for the 13 enforcement check types that Paradigm
 * surfaces during doctor, postflight, and compliance review passes. Narrations
 * explain WHY a gap matters and HOW to fix it — not just what was detected.
 *
 * Used by: paradigm doctor --explain, paradigm_ambient_learn_postflight
 */
...
export interface CheckResult {
  ...
  type: CheckType;
  /** The file, symbol, or resource that triggered the gap */
  target?: string;
  /** Severity: blocking stops a commit, improvement is advisory */
  severity?: 'blocking' | 'improvement' | 'note';
  /** Raw details from the check (e.g., list of affected files) */
  details?: string[];
  /** Any context the checker wants to pass to the narrator */
  context?: Record<string, unknown>;
}

export interface NarrationReport {
  /** Total number of gaps narrated */
  gapCount: number;
  /** Gaps grouped by severity */
  blocking: NarratedGap[];
  improvement: NarratedGap[];
  note: NarratedGap[];
  /** Full narrative text for display */
  narrative: string;
}
...
/**
 * Produce a human-readable narration for a single check result.
 *
 * @param checkType - The type of check that failed
 * @param checkResult - The full check result (may include target, details, context)
 * @returns Narration string explaining the gap and how to fix it
 */
...
/**
 * Produce a full narration report for an array of check results.
 *
 * Groups results by severity (blocking → improvement → note) and renders
 * a complete narrative suitable for display in doctor --explain output or
 * the postflight report.
 *
 * @param checkResults - Array of check results to narrate
 * @returns NarrationReport with grouped gaps and a full narrative string
 */
...
function buildNarrativeText(
  ...
  lines.push(`Found ${total} gap${total !== 1 ? 's' : ''} across ${countTypes(blocking, improvement, note)} check type${countTypes(blocking, improvement, note) !== 1 ? 's' : ''}.`);
  lines.push('');
