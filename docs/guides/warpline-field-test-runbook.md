...
Status: DRAFT (2026-08-23). Companion to the LOCKED pre-registration
`.paradigm/research/warpline-native-first/expo-field-test-protocol.md` — the protocol is the
law; this is the checklist. **The run may not start** until the founder gates in
`T-2026-08-23-001` are closed (M3 per TD-913(3) built or superseded; subject repo consented;
pre-registration v2 issued for the §A/F3 contradictions). Readiness analysis:
`.paradigm/research/warpline-native-first/field-test-readiness-2026-08-23.md`.

Tooling status legend: ✅ built · 🔨 increment 1 (in progress) · ⏳ increment 2 (T-2026-08-23-003).

## 0. Pre-run gates (all must be checked, in writing, before admission 1)

- [ ] Founder gates F1–F7 closed (`T-2026-08-23-001`) — including pre-registration v2 with its
      own id for: the intervention/floor contradiction, structured-output wording, the
      audit-sample rubric variant, the precision-correction formula, TD-831 sampling rule, HELD.
- [ ] Subject repo prepared: `npm install`; `npx tsc --noEmit` GREEN at base (fix or `exclude`
      the `admin/` sweep first); `npx expo export` succeeds; declared green-gate set frozen in
...
- [ ] Agents launched with the pinned model: `claude --model claude-opus-5` (protocol §2).
      Record the launch command in the run log.
- [ ] `ANTHROPIC_API_KEY` present ONLY in the judge operator shell — never in any agent env.
      `@anthropic-ai/sdk` installed for the judge process.
- [ ] Seeds sealed BEFORE the run: planted false-CLEAN control (A6), KNOWN-GENUINE /
      KNOWN-OVER-BLOCK classifier seeds (A11), the externally-authored injection corpus
      (§5 gate 2 — author is not the founder and not the envelope author; membership sealed).
...
- Same-screen pairs (Z1) are the dominant over-block generator on this subject; if the
