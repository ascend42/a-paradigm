...
## Amendment 2026-07-02 — trust-floor SCOPE CORRECTION (adversarial re-review + live exploits)

The Move 2 bundle above defeats **single-file, hash-preserving tamper of v2 strands** — but a follow-up adversarial review, with two exploits reproduced live, established that it does **NOT** close the v1 prefix. Recording the honest boundary so no future reader mistakes A1/A2 for "the fabric is authenticated":

- **HIGH-A — v1 `binding` is unauthenticated (T-2026-07-02-003).** Both the v1 pickId rule (§1.4) and `computeLegacyBodyHash` (A2) **exclude `binding`**. `rewriteFabric` therefore permits adding/swapping a binding on any v1 strand, and verify step 4 only proves the pointed-to tree *recomputes* — an attacker's own store tree recomputes fine. `select`→`restore` then materializes it. **Reproduced:** inject a binding onto v1 seq 8 → `fabric verify` exit 0 ("all intact") → `restore` writes attacker-chosen files (partial, non-atomic; aborted only on an incidental `.warpline` entry name). The A1 gitOid↔treeSha check does not save this — both fields live outside every hash and it is skipped when `gitOid` is null.
- **HIGH-B — the legacy manifest has no integrity of its own (T-2026-07-02-004).** `fabric-legacy.json` is read with format validation but is not itself pinned. **Reproduced:** rewrite grandfathered seq 1's `intent`, recompute `computeLegacyBodyHash` (public code), overwrite the entry's `bodyHash` → verify "all intact" exit 0, count still 7. Body-pinning (A2) only stops tamper that leaves the pin *unchanged*; co-tamper of strand+pin passes. Nothing pins the manifest digest or cardinality (Judge's mint variant: add a `{pickId, bodyHash(tampered)}` entry for a real v1 strand → count 7→8, soft pass).
- **MEDIUM rollup (T-2026-07-02-005):** (C) any non-tip v1 strand's body is content-forgeable by recomputing its `pick:v0` self-hash — only the v1 tip is anchored (first v2 `parentPickId`); the §5/verify "ordering unauthenticatable" caveat understates this. (D) A1 checks recipe trees exist + `result==binding.treeId` but never re-derives `result=merge3(base,ours,theirs)` and never cross-checks `base/ours/theirs` against the parent / merge-parent bindings — a merge strand's claimed inputs are rewritable post-seal. (E) `snapshotRef` skips only `.git`/`.warpline`, not `node_modules` (T-033 asymmetry), and `binding.gitOid` is stored equal to the field verify compares it against, so the step-4 gitOid check can never fire on self-produced strands.

**Root cause (single):** the v1 prefix is anchored only at its **tip**; every interior v1 strand's body and binding is malleable, and the grandfather manifest is an unauthenticated trust root over it. **Honest posture: the v2 chain is trustworthy; the v1 prefix is tamper-evident only against single-file hash-preserving tamper.** Proposed fix (Arky to design): a chained v2 **anchor-attestation** strand that commits to a digest of the entire v1 prefix (bodies + bindings) **and** the legacy manifest, so verify can authenticate the prefix against the chain instead of against self-hashes. Until then, `restore` of a v1 selector should be treated as unverified.

**DESIGNED 2026-07-03:** the fix above is now fully specified in `docs/specs/warpline-v1-anchor.md` (freeze → backfill → attest-once → verify-against-chain; closes HIGH-A/HIGH-B/MED-C). The successor identity model (position-free DAG pickIds, exchangeable strands) is specified in `docs/specs/warpline-v3-identity.md`. This section remains as the honest historical record of the gap.

**BUILT 2026-07-03:** the v1-anchor is implemented — `packages/warpline/src/fabric/anchor.ts` (digests + `findAnchor`/`assertV1Covered`/`attestFabric`), `backfill.ts` (`objects backfill`), the §6 verify walk + §6.7 selvage cross-check (`verify.ts`), the §7 freeze (`fabric.ts` `rewriteFabric` + `grade.ts`), the §8 restore gate (`select.ts`), and the CLI verbs `objects backfill` / `fabric attest`. **v1-prefix authentication in this doc (§3/§5/OQ-A) is SUPERSEDED by warpline-v1-anchor.md** — verify now authenticates the v1 prefix against the chained anchor, not per-strand self-hashes, and hard-requires coverage. Exploit-regression proof: `packages/warpline/test/fabric-anchor.test.ts`.
