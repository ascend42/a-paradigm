...
- **Habits System — Behavioral Feedback Loop** — Agent discipline through observation, measurement, and feedback rather than instructional rulebooks. Core engine in `packages/paradigm/src/core/habits/` with types, YAML loader (project + global + seed merge with overrides, 30s TTL cache), and evaluator (6 check types: tool-called, file-exists, lore-recorded, symbols-registered, gates-declared, tests-exist). 10 seed habits ship built-in across 6 categories (discovery, verification, testing, documentation, collaboration, security) and 4 trigger points (preflight, postflight, on-commit, on-stop). Three severity levels: advisory, warn, block. Sentinel extended with `practice_events` table (v1→v2 schema migration) for tracking habit compliance over time. Three MCP tools: `paradigm_habits_check` (evaluate + record practice events), `paradigm_habits_status` (practice profile with compliance rates by category/habit), `paradigm_practice_context` (proactive warnings before modifying symbols). CLI commands: `paradigm habits list` (grouped by trigger), `paradigm habits status` (compliance bar charts), `paradigm habits init` (scaffold `.paradigm/habits.yaml`), `paradigm habits add` (custom habits). PM integration: preflight surfaces active habits + warnings, postflight evaluates compliance. Stop hook Check 8: blocking habits can prevent session completion via `.paradigm/.habits-blocking` marker file. Lore integration: `habit_compliance` data auto-attached to lore entries with compliance rate, counts, and weak areas.
