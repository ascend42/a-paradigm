...
- **Source redirect (Phase B1)** — the generated CLAUDE.md now carries a "Native Memory Hygiene" directive steering agents to write durable facts to their typed home (lore / habits / decisions / tasks) instead of dumping them in native memory. Stops the junk at the source.
- **`paradigm memory sync` (Phase B2) — keep `MEMORY.md` lean without ever touching your curation.** Advisory by default: reports demotable stale *leaf* entries, dedup candidates, and size before/after; `--write` applies. Strict guards learned from a real-file dry-run: it **never** demotes `##` sections or anything active-marked (`⭐` / `START HERE` / `ACTIVE` / `IN PROGRESS` / recent dates / …) — only genuinely graph-stale leaf `###`+ entries. A curated file is a no-op. `--write` backs up to `MEMORY.md.bak-<ts>` first, writes atomically (temp+rename, original intact on any error), preserves your prose verbatim, and demotes-not-deletes (real deletion stays `memory prune`). An opt-in `--projection` injects a compact (≤8, deduped, capped) managed block of durable knowledge from the typed stores; off by default.
- **Platform "Memory" section (Phase C, `#memory-section`)** — a visual ledger in `paradigm serve`: findings grouped by suggestion (prune / route / merge / pin) with per-row score + verdict + rationale + one-click apply, and a sync-preview panel (size before/after, demotions, dedup, opt-in projection, unified diff) with an explicit **"Apply sync"** button — the human accept-path for demotions too risky to automate. A thin view over the existing `buildDigest`/`computeSync` engines (no new memory logic); the four appliers were extracted to return-shaped functions (`applyPrune`/`applyRoute`/`applyMerge`/`applyPin`) with the CLI wrappers preserved byte-identical.

### Security
- **Platform `/api/*` mutations now enforce `^local-only` at request time (`#local-guard`)** — the server bound loopback but sent `Access-Control-Allow-Origin: *` and never validated Origin/Host per request, so any web page the user had open could POST to `http://127.0.0.1:<port>/api/...` and mutate local state (surfaced by the new Memory write endpoints that touch `~/.claude`; the pre-existing task-write routes shared the exposure). A new guard rejects state-changing requests whose `Host` or (present) `Origin` is non-loopback (CSRF + DNS-rebinding), and CORS now reflects only loopback origins instead of `*`. Reads and same-origin UI calls are unaffected. Memory HTTP responses also return basenames instead of absolute `~/.claude` paths, and error bodies are generic.
...
### Fixed
- **`paradigm_search` never matched multi-word / descriptive queries (`#symbol-index`)** — `searchSymbols` tested the *entire* lowercased query as one literal substring against symbol / description / tags / componentType, so any multi-token query (e.g. `"session recovery checkpoint"`) returned 0 results despite the tool advertising name + description + tags + fuzzy matching. The names-only Levenshtein fallback (cap 3) never rescued a phrase either. `searchSymbols` now tokenizes on whitespace, matches entries on ≥1 token across a per-entry haystack, and ranks by matched-token count → symbol-name match → shorter symbol. Single-token queries keep the exact prior substring behavior (no regression). Fix lives in `@a-company/premise-core`; a rebuild is required for the MCP server to pick it up. 8 new tests.
- **`paradigm purpose validate <file>` silently reported "Found 0 purpose files" (`#purpose-validate`)** — the command globbed `**/.purpose` with the argument as the glob cwd, so a *file* path yielded a non-directory cwd → 0 matches → a green pass. It now `stat`s the path: a `.purpose` file is validated directly, a directory globs as before, a missing path or a non-`.purpose` file each emit a clear error and exit 1.
- **Generated MCP config couldn't find `paradigm-mcp` in GUI/non-login launch contexts (`#mcp-config`)** — the generated MCP config launched the server with a bare `command: paradigm-mcp` and no `env`, so when Claude Code spawned it from a PATH-deficient context — the classic macOS GUI/non-login-shell gap where `/opt/homebrew/bin` is missing from PATH — the command failed to resolve and the server never started. For **Claude Code** (whose `${VAR}` expansion in `.mcp.json` is confirmed) the generator now emits an `env.PATH` **append** form, `${PATH}:/opt/homebrew/bin:/usr/local/bin:${HOME}/.local/bin:${HOME}/.npm-global/bin` (Windows: `${PATH};${APPDATA}\npm`), which **augments — never replaces** — the inherited PATH so the global bin dir resolves. `command`/`args`/`cwd` are untouched. **Scope is deliberately Claude-Code-only:** for every other client (Cursor, Cline, Continue, Claude Desktop) `${VAR}` expansion is unconfirmed, and neither augmentation form is safe without it — an unexpanded literal `${PATH}` would *destroy* PATH, and a static absolute list would *replace* it (regressing nvm/Volta users) — so those clients keep the bare command (today's behavior, no regression) until their expansion is verified. The config is a **committed, per-machine artifact** regenerated by `paradigm sync` / `paradigm mcp setup`. Same launch-context-independence class as 7.8.1's project-root fix. New shared `mcpServerEnv()` helper; new tests.
- **Stop-hook false-green when the CLI is absent (`#hooks`)** — every `paradigm` call was guarded by `command -v paradigm … || true`, so a *missing* CLI was byte-identical to a *clean* run. The hook now resolves `PARADIGM_BIN` once (real `paradigm`, else an `npx --no-install` probe that can't auto-install the unrelated public package, else none), emits a loud stderr banner and a fail-closed blocking violation when none is found, and the exit-127 compliance-run branch is gated on availability so a missing CLI reports exactly once with an actionable install hint (a present-but-crashing CLI still trips it). 4 new tests.

### Changed
- **Session-recovery banner annotates the checkpoint's source branch (`#session-tracker`, TD-2026-09-01-136)** — recovery stays deliberately branch-blind (filtering by branch would blank recovery right after a branch switch), but `SessionCheckpoint` now carries the branch it was saved on and the recovery preamble appends `(from branch <saved> — you are now on <current>)` when they differ, making cross-branch checkpoints legible instead of confusing. Also corrected two stale "7 days" docstrings and the mirror `.purpose` aspect to the real 30-day retention window. 4 new tests.
...
- **WS firehose split into channels** (`#serve`, T-2026-06-15-001 → closes #19): the platform WebSocket `broadcast()` hit every client regardless of what they were viewing. Now it's channel-routed — the channel is the segment before `:` in a message `type` (`agent`/`tasks`/`sentinel`/`symphony`/`ambient`) — and a client can `{ type: 'subscribe', channels: [...] }` to narrow to only what it needs. Backward-compatible: a client that never subscribes defaults to all channels (the old firehose), so nothing regresses; the infrastructure is there for selective consumers (e.g. a native Conductor that only wants `tasks` + `agent`).
- **True two-way GitHub sync — the foundation** (`#task-sync`, `#github-provider`): the GitHub link goes bidirectional, and `done` stops being strictly terminal so a reopened issue can reopen a task. (1) **State machine: `done → open` is now a legal REOPEN** — reopening clears the completion/settlement stamps so the learning loop can re-settle cleanly (mistakenly-completed tasks can be reopened too). (2) The `SyncProvider` seam gains `pull()` + `reopen()` + `edit()` and a provider-agnostic `RemoteState` (structured fields only — never the free-text body, closing the injection vector); GitHub flips to `pull: true` and implements them via `gh issue view --json` / `reopen` / `edit`. (3) A **pure `reconcile()` brain** computes the inbound plan — local stays canonical, a remote change only applies when the implied transition is *legal* (`assertTransition`), an illegal one is a surfaced conflict; symmetric reopen drives a remote-open issue back to `open` on a local `done`. (4) A **`sync-layer`** executes plans **only through the enforced `updateTask`/`completeTask`/`shelveTask` writers** — a pull can never bypass the state machine — plus `projectTransition()` for best-effort outbound push. +20 tests across the seam, the reconcile brain, and the reopen path.
- **Two-way sync — wired to the web + a background poll** (`#task-sync`, `#serve`): the web write verbs (claim/start/done/block/unblock) now **fire an outbound GitHub projection** after the enforced local write (fire-and-forget — a `gh` failure never blocks the response or undoes the write; unlinked tasks cost nothing). A new **`POST /api/tasks/sync`** is the real inbound Pull — it reconciles linked issues back through the enforced writers and returns per-task verdicts (synced/agree/conflict/skipped), broadcasting `tasks:synced` so the board refreshes. And a **background poll** in `paradigm serve` (every 120s, `PARADIGM_SYNC_POLL=off` / `PARADIGM_SYNC_POLL_MS` to tune) pulls linked issues on a timer so the board reflects GitHub changes *without* a manual sync — non-overlapping, self-skipping when nothing is linked or `gh` is unavailable, dies with the process.
- **Two-way sync — the CLI + the web button** (`#task-sync`, `#task-cli`): the CLI verbs (`task start`/`done`/`shelve`, `task edit --reopen`) now project outbound to the linked issue too, and a new **`paradigm task sync [ref]`** does the inbound reconcile from the terminal (per-task synced/conflict/unchanged verdicts; no ref = all linked). On the web, the Tasks GitHub strip's button is now a **real Sync** — it POSTs `/api/tasks/sync`, renders the summary (✓ synced · *N* conflict — amber, never red — · unchanged), makes conflicted task ids clickable, and degrades to a calm "GitHub not available" hint when `gh` isn't authed; the board also live-refreshes off the `tasks:synced` WS broadcast (so the background poll's changes appear without clicking). The full loop: drag a card to Done → the issue closes; a teammate closes/reopens the issue → the poll (or Sync) moves your card through the **same enforced state machine**.

- **Board-health banner** (`#tasks-section`): the three Cid/Loid governance detections (DAG integrity, settlement-debt, stale claims) were returned on the board but invisible — now a compact, advisory (amber, never red) banner surfaces them at the top of the board, collapsed to a count summary ("⚠ 2 DAG issues · 1 settlement-debt · 3 stale claims") and expandable to clickable detail rows that open the offending task. Self-hides entirely on a healthy board. Makes "Cid surfaces it" true on the web surface, not just via the MCP captain tool.
- **Stale archetype-claim surfacing** (`#captain-board`, governance T-010): an archetype-claimed task that's still `open` (never started) past a 7-day window is a candidate to release back to unclaimed — the board now flags these as `staleClaims[]` (Cid-owned, advise-only). Human/peer claims are sticky and never flagged; release stays a deliberate act, never automatic.
- **Claimant-accuracy guard at settle** (`#task-settlement`, governance T-013): the learning a settlement promotes must credit the agent that *did* the work. An orchestration epic is typically claimed by a generic `orchestrator`, so settling under it would promote the journal/work-log to the wrong notebook. When the parent's claimant is absent or the generic orchestrator, settlement now re-attributes to the **dominant child archetype** (the one owning the most child tasks), so learnings land in the right agent's notebook.
- **Settlement-debt detection** (`#captain-board`, governance T-012): Loid's "a never-settling subtree is a learning blindspot" protocol. The board now flags runs whose children are *all* terminal but whose epic never stamped `settledAt` — work that finished but never closed the learning loop. Detect-only (`settlementDebt[]` on the board); Loid surfaces it, Cid owns any graph fix. Settlement writes `settledAt` only, never status (the one-way arrow stays intact).
- **`blocked_on` auto-clear** (`#task-loader`, governance T-008): a `blocked_on` reason was set by Cid's `advance` but nothing ever cleared it — the CLI/board showed a task as blocked until a manual edit. Now, when a task goes terminal, any dependent that was blocked AND whose `dependsOn` are *all* now terminal has its `blocked_on` cleared automatically. Conservative: only clears when there's a real structural dependency edge to the just-finished task (a `blocked_on` set for a non-dependency reason is left for a human to clear); gated on the terminal transition so it can't recurse. +3 tests.
- **Aggregate-on-settle — the calibration loop closes itself** (`#calibration`, governance T-011): learned story-points only refreshed when a human ran `paradigm calibrate` by hand — the actuals captured during orchestration piled up while the table the Tasks moat renders went stale. The aggregation (read actuals → group by archetype×taskType → p10/p90 bands at n≥8) was extracted from the CLI into a shared `paradigm-mcp` util (`calibration-aggregate.ts`, so the settlement chain can call it — MCP can't import the CLI), and `runSettlementChain` now folds it in as a best-effort adjacent step: the moment a task subtree settles, the learned table is rebuilt, so estimates stay current automatically. `paradigm calibrate` keeps its rich report but now shares the one write path. +5 tests (the learned-graduation path proven end-to-end on disk). The mtime-cached loader picks the refresh up transparently.
- **Tasks section — review polish batch 2** (`#tasks-section`): four more team-review refinements. A **collapsed calibration strip now shows a one-line "{pct}% · {graduated}/{total} learned" summary** and force-expands on a cold-start project (the moat is never fully hidden). **Cold-start CellGrid cells now ramp by progress-to-graduation** — a cell with `n=5` actuals looks visibly closer to learning than an empty one, with an in-cell "{n}/8" tick, so the grid reads as a live map of where calibration is forming, not a dead texture. **`proposedClaimant` is surfaced** as a subtle "→ owner" hint on unclaimed cards (Cid's suggestion, distinct from a real claim), and an empty Unclaimed lane is hidden rather than shown as an alarming orange void. **Cross-run `dependsOn` ids now resolve** via `/api/tasks/:id` (deduped, cached) so the Inbox dependency chain stops rendering real deps as "unknown". Also fixed a latent crash — `blocked_on` (a reason string) was mistyped as an array and `.join()`-ed, which would throw whenever a blocked task rendered.
- **Full-forest captain board** (`#captain-board`, governance T-009): the board was run-centric — it only surfaced orchestration epics + their children and the ripple-ranked unclaimed pile, so a human/CLI-**claimed standalone task** (no orchestration parent) was *invisible* on the board. The swim-lanes literally couldn't show claimed work. Added a `loose` category to `assembleCaptainBoard` — non-terminal root tasks that are neither a run nor unclaimed (claimed standalone + in-progress unclaimed standalone) — which the Tasks board renders as claimant lanes (Cid's "see the whole forest" protocol, TD-2026-06-14-467). Loose nodes carry the same estimate + taskType enrichment as run nodes.
- **The agent face gets enforced write verbs** (`#tasks-section`, `#task-loader`): the Inbox and detail panel can now **claim / start / done / block / unblock** a task — the change the team review ranked highest for making the agent-experience first-class instead of read-only. Crucially, this does **not** bypass enforcement: a new write router exposes narrow `POST /api/tasks/:id/{verb}` endpoints that each **proxy the same `updateTask` state-machine path** the CLI/MCP use, so `assertTransition` and the settlement/learning chain fire identically — a browser POST is just another caller of the one enforced writer. The claim verb carries the captain's guard (an archetype claim can't displace a human/peer claim → 409), illegal transitions 409, and a successful settle broadcasts a live `agent:annotate` badge on the task's symbol so the board reflects it in real time. Verbs are status-aware (a verb that would 409 isn't shown), and "Claim to me" resolves the human ref via `/whoami`. Routes declared `^write-capable` in `portal.yaml` (distinct from the read router's `^read-only`).
- **Tasks section — team-review fixes** (`#tasks-section`, `#calibration`): a 6-lens design review of the shipped section drove a batch of high-impact-per-effort fixes. **`taskType` is now plumbed end-to-end** — `/api/tasks` and `/board` attach the classified family per task, so the calibration cell→board filter (the moat's headline interaction, a guaranteed dead-end before) actually works; the card now names the producing `archetype×taskType` cell so an estimate is auditable against the grid. **Confidence drives type scale** — a learned story-point keeps the big green treatment, a cold-start `~prior~` shrinks and leads with its label (the loudest pixel is now always the most trustworthy one). **Claimant lanes resolve to their Team color** (no more hashed hues for named agents) with a widened distinct fallback. **The cold-start strip now names the way out** (`paradigm calibrate`). **GitHub strip honesty** — the un-performable "Pull" button is relabeled "Sync via CLI ↗" and the always-0 "drifted" stat is cut. Human-claimant inboxes no longer assert a calibration % (a category error — learned bands are archetype-keyed). The captain board's governance backlog is now claimed by its owners, so the swim-lanes demonstrate real multi-claimant load.
- **`loadLearnedTokenTable` is mtime-cached** (`#calibration`): the Tasks surface read the learned table 3× per 10s poll per tab (uncached `readFileSync`+`JSON.parse` on the event loop); now cached by file mtime, reloading transparently on any `paradigm calibrate` write.
- **DAG integrity validator** (`#task-loader`, `#captain-board`): the first governance-backlog item (T-2026-06-14-007). `assertTransition` validated task *status* but nothing validated the *graph* — a cyclic or dangling subtree never settles and the learning loop never closes on it. New `validateTaskDag()` detects self-parent, dangling `parentTaskId`/`dependsOn` (refs to nonexistent tasks), and cycles (iterative DFS over parent ∪ dependsOn edges, each cycle reported once, diamonds safe). Cid surfaces violations on the captain board as an advise-only `integrity[]` field — never auto-edits, never blocks (per TD-2026-06-14-467). +7 tests.
- **The Tasks section — Inbox/agent face, live agents, GitHub strip + detail panel** (`#tasks-section`): the first cut, completed. An **Inbox tab** is the agent face of the same task object — a claimant pill-row (Me, resolved via a new `GET /api/tasks/whoami`, + one pill per agent) drives a focused single-claimant column with a rolled-up calibration header, a NEXT-ACTIONABLE hero card, a dependsOn chain ribbon, and "inbox zero" empty states. **Live agents**: each card registers by its derived symbols and consumes the existing agent-effect store (`useTaskAgentEffect`), so an agent's `highlight`/`annotate` pulses/badges the matching card — the board is now somewhere Cid can drive attention live. A **GitHubSyncStrip** footer aggregates per-task sync state with an affordance-only Pull (copies the `paradigm task sync` CLI line — the read router never mutates). A right-slide **TaskDetailPanel** opens on card click: full dependsOn DAG, estimate+provenance breakdown, symbol chips, GitHub section, related lore. The `/api/tasks/*` routes are declared read-only in `portal.yaml` (`^local-only`, `^read-only`), matching the existing platform routes.
- **The Tasks section — Board view, swim lanes + the calibration hero strip** (`#tasks-section`): the spine and the moat-proof. The Board (now the default view) renders **people swim-lanes** — one lane per claimant, colored to match their Team-section identity (agents-as-teammates), each lane header showing name · count · **summed story-points**, plus trailing Unclaimed and Done lanes. A LaneModeToggle regroups the same cards by Claimant | State | Symbol/Flow (persisted). A FilterBar (status/priority/search) narrows the board. Above it sits a **default-on, collapsible calibration hero strip** — an archetype×taskType CellGrid where graduated cells glow solid in agent-color (●) and cold-start cells dim to `░`, with a coverage StatCard ("calibrated cells {pct}% · {graduated} of {total} graduated"). Renders honestly cold-start: an all-`░` grid with a "learning in progress" badge and an all-Unclaimed board, both styled to read as intentional. TaskCard + CalibrationBlock from the prior round render verbatim on the board.
- **The Tasks section — scaffold + List + the calibration moat** (`#tasks-section`): the spearhead surface, first slice. A new Platform section (`▤ Tasks`) following the established lazy-section pattern — header band with a live `{n} open · {n} in-flight` stat + a Board|List|Inbox ViewSwitcher (List wired this round; Board/Inbox next). The headline is real on every card: a **CalibrationBlock** renders the task's story-point (a tunable token-band→Fibonacci mapping) with a green `◆ LEARNED n{count}` badge when the estimate is learned from actuals, or a dimmed `~prior~` when it's a cold-start guess — the learned-vs-prior contrast IS the moat made visible. TaskCards clone the `.lore-card` aesthetic, color claimants via the Team section's `AGENT_COLORS` (agents-as-teammates), and badge GitHub-linked tasks. Design chosen from a three-direction team design pass: "Crew Board" spine + calibration hero strip + scoped Inbox lens.
- **Learned-calibration estimates, surfaced** (`#calibration`, `#tasks`): the planner's `resolveAgentEstimate` always *had* the sample count `n` and learned-vs-prior provenance in its `LearnedBand` cells — it just discarded them, returning bare `{min,max}`. Widened it to `{min, max, n, source:'learned'|'prior'}` (additive — `{min,max}` callers unaffected) and added `estimateForTask()` (resolves the responsible archetype → classifies the blurb → looks up the band) and `assembleCalibrationGrid()` (the archetype×taskType grid + coverage). `/api/tasks` and `/api/tasks/board` now attach a computed `estimate` per task, and a new `/api/tasks/calibration` serves the grid. This is the data spine for the Tasks section's headline feature — story-points = estimates *learned from the team's own actuals*. +8 tests. (Renders honestly cold-start: with no `token-estimates.json` every cell is `source:'prior'`.)
...
- **Provider-agnostic external sync, GitHub out of the gate** (`#sync-provider`, `#github-provider`): tasks can now project to an external tracker — one-way (task → issue), with the local YAML store remaining the **canonical source of truth**. A thin `SyncProvider` seam (designed to be agnostic-*ready* for GitLab/Jira/others, with **GitHub the only implemented provider**) shells `gh` — no OAuth, no server, no token in config. `task push` creates/links a GitHub issue and writes the link into the generalized `external_ref` (`{kind}`→`{provider}`, healed on read); `task link` records a link with no network. **The symbol-aware wedge** — the reason to use this over a raw tracker: `task done` runs **ripple** and prints what else just got fragile, and a commit's `Symbols:` trailer drops a comment on the linked issue (via a best-effort, provider-agnostic post-commit delegate that never blocks a commit). Claimant projection is lossy-by-design: a human claimant → the authenticated pusher (`@me` — GitHub assignees are logins, not emails); an agent/peer claimant → unassigned, never a false human assignment. (Tags + claimant ride in the issue body; arbitrary tags-as-labels are deferred since `gh` rejects labels that don't already exist. Verified live against the repo before release.) **Fully local-first and additive**: with no provider configured everything works unchanged and `push` is a clean no-op; the core never imports provider code; two-way sync and GitLab/Jira impls are gated until a real asker appears.
...
- **The notebook learning loop actually closes** (`#nomination-engine`): a real open-loop bug — `addNotebookEntry` never persisted `confidence` despite the type requiring it, so a promoted entry's confidence was silently absent. Promotion now writes the entry's `confidence_after` to the notebook (latest measurement wins — deliberately **no** `max()` ratchet, so a belief can still be revised downward later). And a new **promotion-decisions instrument** (`.paradigm/events/promotion-decisions.jsonl`) records `{before (the agent's real notebook prior), after, delta, promoted, priorFound}` on every live promotion decision. *The belief-delta promotion gate itself was deliberately **not** shipped* — an adversarial review found it unfalsifiable (no signal measures whether a promotion was correct, so its bands can't be calibrated) and its ratchet broke downward revision. So v7.1 ships the **prerequisites** — the closed loop + the measurement — and defers the gate until the instrument has real data to set bands from a histogram, not intuition (`T-2026-06-13-012`). The proven absolute `≥0.8` gate runs unchanged.
- **The Symphony/peer path joins the loop** (`!symphony-status-flowback`): the consumer side the emission round deferred. When `paradigm_symphony_poll` delivers a `progress`/`task-complete`/`task-failed` note carrying `metadata.task.taskId`, it now maps it to a local task update (`in-progress`/`done`/`shelved`) — so a peer or Cursor agent reporting completion drives settlement + the learning chain exactly as the MCP and CLI paths do. Best-effort and local-only: a note for a task this machine doesn't have is a clean no-op (the parked multi-machine case), and no flow-back failure can affect `poll`'s behavior.
